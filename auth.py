...
.stApp {
    background: #080b12;
    background-image:
        radial-gradient(ellipse 80% 50% at 20% 0%, rgba(0,100,255,0.07) 0%, transparent 60%),
        radial-gradient(ellipse 60% 40% at 80% 100%, rgba(123,47,255,0.07) 0%, transparent 60%);
}
...
.stButton > button:hover {
    background: linear-gradient(135deg,rgba(0,212,255,0.18),rgba(123,47,255,0.18)) !important;
    border-color: #00d4ff !important;
...
def show_login_page():
    # If not on login page, stop rendering immediately
    ...
    with left:
        ...
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:42px;
                    color:#e8f0f8;letter-spacing:-1px;line-height:1;">⬡ HabitOS</div>
        <div style="font-family:'Space Mono',monospace;font-size:10px;letter-spacing:4px;
                    text-transform:uppercase;color:#445566;margin-top:6px;margin-bottom:32px;">
            Health Analytics Platform</div>
        """, unsafe_allow_html=True)

        # Only show value props when popup is NOT open
        if not popup:
            st.markdown("""
        ...
        # Demo popup — right below the buttons
        if popup:
            st.markdown("""
            <div class="demo-popup">
                <div style="font-family:Syne;font-weight:800;font-size:15px;
                            color:#e8f0f8;margin-bottom:4px;">
                    Try with demo data
                </div>
            ...
            if popup == "household":
                st.markdown("""
                <div style="font-family:Inter;font-size:12px;color:#8899aa;margin:10px 0;">
                🏠 Ram · Mathi · Guna — 3 weeks of real habit data
                ...
                st.markdown("""
                <div style="font-family:Inter;font-size:12px;color:#8899aa;margin:10px 0;">
                🧍 Mathi — personal habit tracking dashboard
                </div>""", unsafe_allow_html=True)
    ...
    with gap:
        ...
        <div style="display:flex;justify-content:center;padding-top:80px;">
            <div style="width:1px;height:420px;background:linear-gradient(
                180deg,transparent,#1e2d40 20%,#1e2d40 80%,transparent);"></div>
        </div>
        """, unsafe_allow_html=True)
...
def show_invite_page():
    ...
    with col:
        ...
        st.info("Send this code to your household members when they sign up.")
        st.markdown("<br>", unsafe_allow_html=True)
